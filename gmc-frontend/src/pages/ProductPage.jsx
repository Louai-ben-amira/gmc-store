...
function SectionLabel({ children }) {
  return (
    <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6875rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 12 }}>
      {children}
    </p>
  )
}
...
export default function ProductPage() {
  ...
  const handleBuyClick = () => {
    ...
    if (product?.has_variants && !selectedVariant) {
      toast.error('Please select an amount before purchasing.')
      return
    }
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      if (phoneField) {
        ...
        toast.success('Order placed! Track it in My Orders.'); navigate('/orders')
          ...
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <div className="product-detail-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '2rem', alignItems: 'start', marginTop: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                {/* HOW IT WORKS */}
                <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '1.25rem 1.5rem' }}>
                  <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6875rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-muted)', margin: '0 0 1rem' }}>{t('product.howItWorks')}</p>
                  ...
                  {isAuthenticated() && eligibility?.can_review && (
                    <div style={{ background: 'rgba(124,58,237,0.06)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: 10, padding: '1rem', marginBottom: '1.25rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
                        ...
                        <span style={{ color: 'var(--accent)', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.07em' }}>{t('product.verifiedPurchase')}</span>
                      </div>
                      ...
                      {reviews.slice(0, 6).map((r, i) => (
                        <div key={r.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '0.875rem', background: 'rgba(255,255,255,0.02)', borderRadius: 10, border: '1px solid rgba(255,255,255,0.04)', animation: `fadeSlideUp 0.35s ${i * 0.04}s ease both` }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6875rem', color: 'var(--text-muted)', letterSpacing: '0.06em' }}>
                ...
                {activeVariants.length > 0 && (
                  <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '1.25rem' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                      <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', fontWeight: 800, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>
                        {t('product.selectAmount')}
                      </span>
                      {selectedVariant && (
                        <button onClick={() => setSelectedVariant(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6875rem', color: 'var(--text-muted)', letterSpacing: '0.06em', padding: 0 }}>
                          ✕ CLEAR
                        </button>
                      )}
                    </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span style={{
                              ...
                              {parseFloat(v.price).toFixed(2)}<span style={{ fontSize: '0.75rem', marginLeft: 2, fontWeight: 600 }}>DT</span>
                            </span>
                ...
                <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
                  ...
                  {allowQuantity && maxQty > 1 && !(activeVariants.length > 0 && !selectedVariant) && (
                    <div style={{ padding: '0.875rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>
                  ...
                  <div style={{ padding: '1rem 1.5rem' }}>
                    <button
                      ...
                      style={{
                        width: '100%', padding: '0.9375rem', fontSize: '1rem', fontWeight: 700, justifyContent: 'center', borderRadius: 10,
                        ...(unverified ? { opacity: 0.5, cursor: 'not-allowed' } : {}),
                      }}
                      disabled={!inStock || loading || unverified}
